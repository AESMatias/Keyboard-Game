...
let initialTime: number = Date.now();
let writtingWords = String('')
let writingIndexRight = 0
let writingIndexWrong = 0
let wordsLeft: number = lorem_text.split(' ').length - 1
let finishedStatus = false

let arrayOfRightAttempts: Number[] = [];// Contains all the right attempts in a row
...
const wordToWrite: HTMLElement | null = document.getElementById("word-to-write")!;
wordToWrite.textContent = lorem_text;

let actualWord: HTMLElement = document.getElementById("actual-text")!;
let currentWritedWord = document.getElementById("current-writed-word");
let newWritedWord: string | null = currentWritedWord!.textContent!.slice(0, -1);

// We get the words left paragraph
let wordsLeftParagraph: HTMLElement | null = document.getElementById("words-left")!;
...
document.addEventListener("keydown", function (event) {
    ...
    if (checkKey(pressedKey, writingIndexRight)) {
        ...
        if (writingIndexRight == lorem_text.length) {
            ...
            const timeElapsed: number = (finalTime - initialTime) / 1000;
...
function currentWrited(pressedKey: string) {
...
function checkWordInArray(letter: string, arrayIndex: Number) {
...
function checkKey(key: string, index: number) {
